"""
Train an optimized SVM baseline for tomato leaf disease classification.

Workflow:
- Load train/val images from the split dataset.
- Resize to a fixed size, flatten to 1D feature vectors.
- Encode class labels.
- Train an SVM classifier (with feature scaling and class balancing).
...
"""
“After observing overfitting with the baseline SVM, 
a limited and principled optimization was applied 
by adjusting regularization and class weighting. 
While minority class recall improved slightly,
 overall generalization remained limited,
 motivating the transition to CNN-based models.”
"""
...
MODEL_PATH = Path("experiments/svm/02_svm_model.joblib")

# Image processing
IMAGE_SIZE = (64, 64)  # (width, height)

# Optimized SVM config (minimal, principled optimization)
SVM_KERNEL = "rbf"
SVM_C = 1.0                 #  softer regularization to reduce overfitting
SVM_GAMMA = "scale"
...
def main() -> None:
    ...
    # Build pipeline: scale features then optimized SVM classifier
